import { MapPinIcon, NavigationArrowIcon, CarIcon } from "@phosphor-icons/react/dist/ssr";
          ...
          <div className="rounded-3xl border border-border bg-card p-8 shadow-sm sm:p-10">
            ...
            <ul className="mt-6 flex flex-wrap justify-center gap-2.5">
              {communes.map((commune, i) => (
                ...
                >
                  <MapPinIcon className="size-3.5 text-brand-500" />
                  {commune}
                </li>
              ))}
            </ul>
            <div className="mt-8 flex items-start justify-center gap-3 rounded-2xl border border-brand-100 bg-brand-50/50 px-5 py-4 text-left">
              <CarIcon className="mt-0.5 size-5 shrink-0 text-brand-600" />
              <p className="text-sm text-foreground">
                <span className="font-semibold">Déplacement à domicile offert</span> dans un rayon
                de {deplacement.rayonGratuitKm} km autour d&apos;{site.ville}. Au-delà, un forfait
                fixe de {deplacement.forfaitAuDela} € seulement, annoncé à l&apos;avance et sans
                surprise.
              </p>
            </div>
